...
learn_gen.save('gen-model-'+str(i)+'-resnet50')
...
dls_crit = get_crit_dls(fnames, bs=16, size=64)
...
learn_crit.save('critic-model-'+str(i)+'-resnet50')
...
ls_crit = get_crit_dls(fnames, bs=16, size=64)


# In[28]:


learn_crit = create_crit_learner(dls_crit, metrics=None).load('critic-model-'+str(i)+'-resnet50').to_fp16()


# In[29]:


learn_gen = create_gen_learner().load('gen-model-'+str(i)+'-resnet50').to_fp16()
...
learn.save('gan-model-'+str(i)+'-resnet50')
...
with open('resnet50.txt',mode="a") as f:
